...
const isWeightSelected = ref(Boolean(props.selectedTeam.weight)),
...
function handleSelectedWeight(val) {
	...
	if (away.value.isPicked) {
		selectedTeamsStore.updateSelectedTeams({ team: away.value.team.team_key, weight: selectedWeight.value, game: props.game.global_game_id });
	}
	if (home.value.isPicked) {
		selectedTeamsStore.updateSelectedTeams({ team: home.value.team.team_key, weight: selectedWeight.value, game: props.game.global_game_id });
	}
}
...
<template>
	<div class="m-2 p-2 game w-64 text-lg rounded shadow-sm border border-gray-400 bg-slate-400"
		...
		<p class="mb-0 text-center"><small v-if="hasWeightError" class="text-red-600">
				<TriangleAlert class="h-5 w-5 text-red-600 inline"/>Weight must be
				unique
			</small></p>
		<p class="mb-0 text-center"><small v-if="teamSelectedWeightZero" class="text-red-600">
				<TriangleAlert class="h-5 w-5 text-red-600 inline"/>Weight should not
				be zero
			</small></p>
		...
		<div class="grid gap-2">
			<Dialog>
				<DialogTrigger
					...
					Game Details</DialogTrigger>
				<DialogContent>
					<DialogHeader>
						<DialogTitle class="py-2 border-b">
							<h5 class="mb-0 modal-title">{{ game.away_team.team_key }} vs. {{ game.home_team.team_key }}</h5>
							<small class="text-xs text-slate-500">Game ID: {{ game.global_game_id }}</small>
						</DialogTitle>
						<DialogDescription class="grid grid-cols-1 md:grid-cols-2 gap-2">
              <dl>
                <dt class="text-white font-extrabold">Stadium</dt>
                <dd class="mb-2">{{ game.stadium_name }}</dd>

                <dt class="text-white font-extrabold">Stadium Type</dt>
                <dd class="mb-2">{{ game.stadium_type }}</dd>

                <dt class="text-white font-extrabold">Playing Surface</dt>
                <dd class="mb-2">{{ game.playing_surface }}</dd>

                <dt class="text-white font-extrabold">Watch On</dt>
                <dd class="mb-2">{{ game.channel }}</dd>
              </dl>
              <dl>
                <dt class="text-white font-extrabold">Weather Forecast</dt>
                <dd class="mb-2">75&deg; and Sunny</dd>

                <dt class="text-white font-extrabold">Betting Odds</dt>
                <dd class="mb-2">Team Name -3.5</dd>
              </dl>
						</DialogDescription>
					</DialogHeader>
				</DialogContent>

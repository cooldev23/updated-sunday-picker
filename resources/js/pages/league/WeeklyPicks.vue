...
import LeagueMemberPick from '@/Components/LeagueMemberPick.vue';
...
function getUserPick(user, game) {
    // console.log(user.picks);
    return user.picks.filter((pick) => pick.game_id = game.global_game_id);
}
            ...
            <div class="py-12">
                <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    ...
                    <Card class="mb-16 border border-gray-400 shadow-sm">
                        <template #header>
                            <div class="p-3 bg-gray-200">Weekly Picks</div>
                        </template>
                        <template #body>
                            <DataTable class="display">
                                ...
                                <tbody>
                                    <tr v-for="member in league.users" :key="member.id">
                                        ...
                                        <!-- TODO: split this into two different td elements -->
                                        <td>
                                            <div v-if="league.league_type_id !== 2">
                                                <div v-if="$page.props.auth.user.id === member.id || leagueCanShowGames" class="flex items-center">
                                                    <LeagueMemberPick v-for="game in weeksGames" :game="game" :user="member"></LeagueMemberPick>
                                                    <!-- <p v-for="pick in member.picks" class="mx-1 w-11 h-16 shadow-sm flex flex-col justify-center items-center rounded-sm" :class="{'border border-red-400 text-red-400': !pick.correct}, {'border border-green-600 text-green-600': pick.correct}">
